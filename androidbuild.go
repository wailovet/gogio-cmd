...
<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
	...
	manifestTmplSrc, _ := ioutil.ReadFile("manifest.tmpl")
	manifestTmplStr := string(manifestTmplSrc)
	if manifestTmplStr == "" {
		manifestTmplStr = `<?xml version="1.0" encoding="utf-8"?>
		...
		ioutil.WriteFile("manifest.tmpl", []byte(manifestTmplStr), 0660)
	}

	tmpl, err := template.New("test").Parse(manifestTmplStr)
